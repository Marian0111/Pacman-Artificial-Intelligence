...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        return self.startingPosition, [False for c in self.corners]
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                if (successorX, successorY) in self.corners:
                    successorCornerState[self.corners.index((successorX, successorY))] = True

                successors.append((((successorX, successorY), tuple(successorCornerState)), action, 1))
...
def cornersHeuristic(state, problem: CornersProblem):
    ...
    return max(manhattanDistance(state[0], corner) for corner in problem.corners if not state[1][problem.corners.index(corner)])
